...
function createExportService({ configStore } = {}) {
  async function exportWordInternal(payload, filePath, onProgress) {
    ...
    try {
      ...
      developerLogger.write('export.word.write.started', {
        output_file_name: path.basename(filePath),
        output_extension: path.extname(filePath).toLowerCase(),
        buffer_bytes: buildResult.buffer.length,
      });
      fs.writeFileSync(filePath, buildResult.buffer);
      ...
      return { success: true, path: filePath, message, warnings: buildResult.warnings };
    } catch (error) {
  ...
  return {
    async exportWord(payload = {}, onProgress) {
      ...
      return exportWordInternal(payload, result.filePath, onProgress);
    },

    async exportWordToPath(payload = {}, filePath, onProgress) {
      return exportWordInternal(payload, filePath, onProgress);
    },
  };
}
