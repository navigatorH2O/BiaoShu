...
const GITHUB_RELEASE_API = 'https://api.github.com/repos/navigatorH2O/BiaoShu/releases/latest';
const GITHUB_RELEASE_DOWNLOAD_URL = 'https://github.com/navigatorH2O/BiaoShu/releases/latest';
...
function requestJson(url, label, headers = {}) {
  return new Promise((resolve, reject) => {
    const request = https.get(url, { headers: { 'User-Agent': 'biaoshu-client', ...headers } }, (response) => {
...
function getMacDmgDownloadPath(app, release, file) {
  const fallbackName = `BiaoShu-${release.version || 'update'}-mac-${getMacUpdateArch()}.dmg`;
...
function downloadFile(url, destinationPath, options = {}, redirectCount = 0) {
  ...
  return new Promise((resolve, reject) => {
    ...
    let request;
    try {
      request = requestModuleForUrl(parsedUrl).get(parsedUrl, { headers: { 'User-Agent': 'biaoshu-client' } }, (response) => {
        if (response.statusCode >= 300 && response.statusCode < 400 && response.headers.location) {
          response.resume();
          if (redirectCount >= 5) {
